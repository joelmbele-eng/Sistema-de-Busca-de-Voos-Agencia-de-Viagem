...
class SistemaVoos(ctk.CTk):
    ...
    def mostrar_rota_mapa(self, origem, destino):
        ...
        # Criar um canvas para desenhar o mapa
        canvas_frame = ctk.CTkFrame(self.mapa_container)
        ...
        try:
            ...
            # Desenhar uma representação simples do mapa
            # Normalizar as coordenadas para o canvas
            min_lon = min(origem_info['coords'][1], destino_info['coords'][1])
            max_lon = max(origem_info['coords'][1], destino_info['coords'][1])
            min_lat = min(origem_info['coords'][0], destino_info['coords'][0])
            max_lat = max(origem_info['coords'][0], destino_info['coords'][0])
            
            # Adicionar margem
            lon_margin = (max_lon - min_lon) * 0.2 if max_lon != min_lon else 10
            lat_margin = (max_lat - min_lat) * 0.2 if max_lat != min_lat else 10
            min_lon -= lon_margin
            max_lon += lon_margin
            min_lat -= lat_margin
            max_lat += lat_margin
            
            # Função para converter coordenadas geográficas para coordenadas do canvas
            def geo_to_canvas(lat, lon):
                # Inverter latitude porque o canvas tem origem no canto superior esquerdo
                x = (lon - min_lon) / (max_lon - min_lon) * 380 + 10 if max_lon != min_lon else 250
                y = (1 - (lat - min_lat) / (max_lat - min_lat)) * 280 + 10 if max_lat != min_lat else 175
                return x, y
            
            # Desenhar linha entre origem e destino
            x1, y1 = geo_to_canvas(origem_info['coords'][0], origem_info['coords'][1])
            x2, y2 = geo_to_canvas(destino_info['coords'][0], destino_info['coords'][1])
            canvas.create_line(x1, y1, x2, y2, fill="blue", width=2)
            
            # Desenhar marcadores
            canvas.create_oval(x1-5, y1-5, x1+5, y1+5, fill="red", outline="")
            canvas.create_oval(x2-5, y2-5, x2+5, y2+5, fill="green", outline="")
            
            # Adicionar rótulos
            canvas.create_text(x1, y1-15, text=origem, fill="black")
            canvas.create_text(x2, y2-15, text=destino, fill="black")
            ...
            ctk.CTkLabel(
                info_frame,
                text=f"🔴 Origem: {origem} - {origem_info['nome']}",
                font=("Roboto", 14)
            ).pack(pady=5, anchor="w", padx=10)
            
            ctk.CTkLabel(
                info_frame,
                text=f"🟢 Destino: {destino} - {destino_info['nome']}",
                font=("Roboto", 14)
            ).pack(pady=5, anchor="w", padx=10)
            ...
            # Adicionar botão para gerar mapa interativo
            ctk.CTkButton(
                self.mapa_container,
                text="Gerar Mapa Interativo",
                command=lambda: self.gerar_mapa_interativo(origem_info, destino_info),
                font=("Roboto", 14),
                fg_color="#007bff"
            ).pack(pady=10)
            ...
            if origem_info and destino_info:
                info_basica = ctk.CTkFrame(erro_frame)
                info_basica.pack(fill="x", pady=10)
    ...
    def gerar_mapa_interativo(self, origem_info, destino_info):
        try:
            # Criar um mapa folium
            mapa = folium.Map(
                location=[(origem_info['coords'][0] + destino_info['coords'][0])/2,
                         (origem_info['coords'][1] + destino_info['coords'][1])/2],
                zoom_start=4
            )
            ...
            folium.Marker(
                origem_info['coords'],
                popup=f"Origem: {origem_info['nome']}",
                icon=folium.Icon(color='red')
            ).add_to(mapa)
            
            folium.Marker(
                destino_info['coords'],
                popup=f"Destino: {destino_info['nome']}",
                icon=folium.Icon(color='green')
            ).add_to(mapa)
            
            # Adicionar linha entre os pontos
            folium.PolyLine(
                locations=[origem_info['coords'], destino_info['coords']],
                weight=3,
                color='blue',
                opacity=0.8
            ).add_to(mapa)
            ...
            self.mostrar_resultado(f"Erro ao gerar mapa interativo: {str(e)}")
    ...
    def mostrar_pdf(self, pdf_path, manter_zoom=False, ajustar_janela=False):
        try:
            ...
            # Mostrar informações sobre o PDF
            ...
            titulo_pdf.pack(side="left", padx=20)
            ...
            self.pdf_canvas.create_text(
                300, 400,
                text=f"Erro ao abrir o PDF: {str(e)}",
                fill="red",
                font=("Roboto", 14)
            )
